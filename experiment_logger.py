...
def logExperimentResult(
    *,
    ...
    results = {
        ...
        "mu_mean": mu.mean().item() if not (mu is None) else None,
        "mu_std": mu.std().item() if not (mu is None) else None,
        "training_set": training_set if not (training_set is None) else None,
        "var_mean": var.mean().item() if not (var is None) else None,
        "var_std": var.std().item() if not (var is None) else None,
        "n_epochs": n_epochs,
        "n_samples": n_samples,
